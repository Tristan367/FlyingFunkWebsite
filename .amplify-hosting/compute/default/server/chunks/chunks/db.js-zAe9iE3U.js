...
class PgColumn extends Column {
  constructor(table, config) {
    if (!config.uniqueName) {
      config.uniqueName = uniqueKeyName$1(table, [config.name]);
    }
    super(table, config);
    this.table = table;
  }
...
var members = sqliteTable("members", {
	...
	unavailableOnHolidays: integer("unavailable_on_holidays", { mode: "boolean" }).notNull().default(false),
...
var verificationCodes = sqliteTable("verification_codes", {
	id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
	email: text("email").notNull(),
	code: text("code").notNull(),
	expiresAt: integer("expires_at").notNull()
});
var unavailability = sqliteTable("unavailability", {
	id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
	memberId: text("member_id").notNull().references(() => members.id, { onDelete: "cascade" }),
	date: text("date").notNull(),
	isAvailable: integer("is_available", { mode: "boolean" }).notNull().default(false)
});
var recurringUnavailability = sqliteTable("recurring_unavailability", {
	id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
	memberId: text("member_id").notNull().references(() => members.id, { onDelete: "cascade" }),
...
var gigVotes = sqliteTable("gig_votes", {
	id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
	gigId: text("gig_id").notNull().references(() => gigs.id, { onDelete: "cascade" }),
	memberId: text("member_id").notNull().references(() => members.id, { onDelete: "cascade" }),
	vote: text("vote", { enum: [
		"approve",
		"reject",
...
var images = sqliteTable("images", {
	...
	uploadedAt: text("uploaded_at").notNull().$defaultFn(() => (/* @__PURE__ */ new Date()).toISOString())
});
var songs = sqliteTable("songs", {
	id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
	uploaderId: text("uploader_id").notNull().references(() => members.id, { onDelete: "cascade" }),
	title: text("title").notNull(),
	description: text("description").notNull().default(""),
...
var removalVotes = sqliteTable("removal_votes", {
	id: text("id").primaryKey().$defaultFn(() => crypto.randomUUID()),
...
var siteConfig = sqliteTable("site_config", {
...
async function createConnection() {
	if (isPostgres) {
		const { drizzle: pgDrizzle } = await import('../index-Bm5Ox5_R.js');
		const postgres = (await import('postgres')).default;
		return pgDrizzle(postgres(dbUrl, { ssl: "require" }), { schema: schema_exports });
	} else {
		const { drizzle: libDrizzle } = await import('../index-BXsdoOOi.js');
		const { createClient } = await import('../node-Pu9Op8J5.js');
		return libDrizzle(createClient({ url: dbUrl }), { schema: schema_exports });
	}
}
function getDb() {
...
var db = new Proxy({}, { get(_target, prop) {
	...
	return (...args) => {
		const p = getDb();
		if (p instanceof Promise) return p.then((d) => {
			const method = d[prop];
...
export { getTableColumns as $, getTableName as A, Param as B, CasingCache as C, PgJsonb as D, PgJson as E, PgNumeric as F, PgTime as G, PgTimestamp as H, PgTimestampString as I, PgDate as J, PgDateString as K, PgUUID as L, aliasedTableColumn as M, getOperators as N, mapColumnsInSQLToAlias as O, PgTable as P, mapColumnsInAliasedSQLToAlias as Q, getOrderByOperators as R, SQL as S, Table as T, normalizeRelation as U, View as V, getTableUniqueName as W, One as X, DrizzleError as Y, Many as Z, aliasedTable as _, eq as a, tracer as a0, TypedQueryBuilder as a1, getTableLikeName as a2, SelectionProxyHandler as a3, haveSameKeys as a4, applyMixins as a5, QueryPromise as a6, WithSubquery as a7, Schema as a8, Columns as a9, mapUpdateSet as aa, mapRelationalRow as ab, NoopCache as ac, DrizzleQueryError as ad, hashQuery as ae, TransactionRollbackError as af, fillPlaceholders as ag, mapResultRow as ah, isConfig as ai, extractTablesRelationalConfig as aj, createTableRelationsHelpers as ak, SQLiteSession as al, SQLitePreparedQuery as am, SQLiteTransaction as an, SQLiteAsyncDialect as ao, BaseSQLiteDatabase as ap, and as b, gigs as c, db as d, entityKind as e, blogPosts as f, gte as g, sql as h, desc as i, siteConfig as j, songs as k, images as l, members as m, ne as n, gigVotes as o, removalVotes as p, is as q, recurringUnavailability as r, sessions as s, PgColumn as t, unavailability as u, verificationCodes as v, Column as w, Subquery as x, ViewBaseConfig as y, orderSelectedFields as z };
//# sourceMappingURL=db.js-zAe9iE3U.js.map
