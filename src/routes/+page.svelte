...
<section class="relative overflow-hidden {data.config.heroImage ? 'bg-zinc-950' : 'bg-gradient-to-b from-amber-500/10 to-zinc-950'} py-32 sm:py-48">
	...
	<div class="relative mx-auto max-w-4xl px-4 text-center">
		<h1 class="mb-4 text-6xl font-black tracking-tight text-amber-400" style="text-shadow:0 0 16px rgba(0,0,0,1), 0 0 4px rgba(0,0,0,1)">{data.config.heroTitle}</h1>
		<p class="mb-10 max-w-xl mx-auto text-base leading-relaxed text-zinc-300" style="text-shadow:0 0 8px rgba(0,0,0,1), 0 0 4px rgba(0,0,0,1), 0 0 4px rgba(0,0,0,1), 0 0 4px rgba(0,0,0,1)">
			{data.config.heroSubtitle}
		</p>
		<div class="flex justify-center">
			<a href="/book" class="rounded-lg bg-amber-500 px-8 py-3 font-bold text-zinc-900 transition-colors hover:bg-amber-400 shadow-lg shadow-black/60">Book Us</a>
		</div>
	</div>
</section>
...
<section class="mx-auto max-w-4xl px-4 py-16">
	<div class="rounded-lg border border-zinc-800 bg-zinc-900 p-8 text-center">
		...
		<a href="/songs" class="inline-block rounded-lg bg-amber-500 px-8 py-3 font-bold text-zinc-900 transition-colors hover:bg-amber-400">Listen</a>
	</div>
</section>
{/if}
	...
	<div class="mx-auto grid max-w-2xl grid-cols-2 gap-4 sm:grid-cols-4">
		{#each data.members as member}
			...
			<a href={hasProfile ? '/' + member.slug : '#'}
				...
				class="rounded-lg border border-zinc-800 bg-zinc-900 p-4 text-center transition-colors {hasProfile ? 'hover:border-amber-500/50' : 'cursor-default'}"
			>
	...
	<section class="mx-auto max-w-4xl px-4 py-16">
		...
		<div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
			{#each data.blogPosts as post}
				<a href="/blog/{post.slug}" class="rounded-lg border border-zinc-800 bg-zinc-900 p-6 transition-colors hover:border-amber-500/50">
